...
def _extract_amount_candidates(lines: list[str]) -> list[int]:
    ...
    for line_index, line in enumerate(lines):
        ...
        has_exclude_keyword = any(
            keyword in line for keyword in _EXCLUDE_AMOUNT_KEYWORDS
        )
...
def extract_receipt_candidates(raw_text: str) -> dict[str, Any]:
    ...
    amount_candidates = _extract_amount_candidates(lines)
    date_candidates = _extract_date_candidates(lines)
    store_candidates = _extract_store_candidates(lines)

    return {
        "amountCandidates": amount_candidates,
        "dateCandidates": date_candidates,
        "storeNameCandidates": store_candidates,
        "topLines": lines[:10],
    }