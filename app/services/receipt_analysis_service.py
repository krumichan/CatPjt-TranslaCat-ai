...
class ReceiptAnalysisService:
    ...
    def _compact_raw_text_for_ai(
        ...
        stop_keywords = (
            tuple(options.stop_keywords)
            if options.stop_keywords is not None
            else _DEFAULT_STOP_KEYWORDS
        )
        ...
        for index, line in enumerate(lines):
            ...
            # 금액/합계/식사 형태 관련 키워드는 유지
            if any(keyword in line for keyword in important_keywords):
                compact_lines.append(line)
                continue

            # 금액처럼 보이는 짧은 숫자 라인은 유지
            if self._looks_like_amount_line(line):
                compact_lines.append(line)
                continue

            # 상품명 후보:
            # 너무 길지 않고, 숫자가 적고, 의미 있는 문자가 있는 라인만 제한적으로 유지
    ...
    def _adjust_confidence(
        ...
        # 금액 후보가 있는데 AI가 후보에 없는 금액을 골랐으면 낮춘다.
        ...
        # 핵심 3개가 모두 있으면 너무 낮게 떨어지지 않게 보정
    ...
    def _sanitize_memo(self, memo: str | None) -> str | None:
        ...
        # 예: "A아이스"처럼 일본어 영수증에서 한국어가 섞인 애매한 값은 제거
